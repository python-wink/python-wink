...
class WinkApiInterface(object):
    ...
    def set_device_state(self, device, state, id_override=None):
        ...
        arequest = requests.put(url_string,
                                data=json.dumps(state), headers=API_HEADERS)
...
def get_devices_from_response_dict(response_dict, filter_key, api_interface):
    """
...
def get_device_state(device):
    ...
    """
    url_string = "{}/{}/{}".format(BASE_URL,
                                   device.objectprefix, device.parent_id())
